...
     """
    st.write("Funktion offene_bestellungen_lesen wurde aufgerufen") # Debugging - Funktionsaufruf # Debugging - Funktionsaufruf
...
        st.write("datei_inspektion_und_anpassung hat None zurückgegeben!") # Debugging - Fehlerfall # Debugging - Fehlerfall
        return None

    st.write("Datentypen des DataFrames (Offene Bestellungen):") # Debugging # Debugging
    st.write(df_offene_bestellungen.dtypes) # Debugging # Debugging
    st.write("Erste Zeilen des DataFrames (Offene Bestellungen):") # Debugging # Debugging
    st.dataframe(df_offene_bestellungen.head()) # Debugging # Debugging
...
    st.write(f"Prüfe Artikelnummer in offenen Bestellungen: {artikelnummer}") # Debugging # Debugging
    bestellungen_artikel = offene_bestellungen_df[offene_bestellungen_df['Artikelnr.'] == artikelnummer] # Spaltenname 'Artikelnr.'
    st.write(f"Anzahl Bestellungen für Artikelnummer gefunden: {len(bestellungen_artikel)}") # Debugging # Debugging
...
        st.write(f"Prüfe Belegnummer: {belegnummer}") # Debugging # Debugging
...
        st.write(f"Alle Positionen 'Geliefert' == 0 für Belegnummer {belegnummer}: {alle_geliefert_null}") # Debugging # Debugging
...
        st.write("Ergebnis von offene_bestellungen_lesen (direkt nach Aufruf):") # Debugging # Debugging
        st.write(offene_bestellungen_df) # Debugging - DataFrame oder None ausgeben # Debugging - DataFrame oder None ausgeben

        if offene_bestellungen_df is not None: # Stelle sicher, dass df nicht None ist, bevor .head() # Debugging -  Bedingung hinzugefügt
            st.write("Erste Zeilen von offene_bestellungen_df (zur Prüfung von Artikelnr. und Belegnr.):") # Debugging # Debugging
            st.dataframe(offene_bestellungen_df.head()) # Debugging # Debugging
